...
class Register(QtWidgets.QMainWindow, project_1_ui.Ui_MainWindow):
    ...
    def register_account(self):
        ...
        if login and password:
            akaunts = read_akaunts()
            if login in akaunts:
                reply = QtWidgets.QMessageBox.question(self, 'Помилка', 'Такий акаунт вже існує. Хочете вибрати інше ім\'я?',
...
class Login(QtWidgets.QMainWindow, project_1_ui.Ui_MainWindow):
    ...
    def show_profile(self):
        if self.logged_in:
            # Get the username from the logged-in account
            username = self.lineEdit.text().strip()
            profiles = read_profiles()
            profile_data = profiles.get(username)
            if profile_data:
    ...
    def login_attempt(self):
        ...
        if user_login and user_password:
            ...
            if user_login in akaunts:
                if akaunts[user_login]['PasswordHash'] == hashlib.sha256(user_password.encode()).hexdigest():
                    self.logged_in = True
